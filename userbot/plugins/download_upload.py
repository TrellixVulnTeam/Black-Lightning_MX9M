...
from userbot.utils import lightning_cmd, humanbytes, progress, register, time_formatter

thumb_image_path = f'{Var.TEMP_DOWNLOAD_DIRECTORY}/thumb_image.jpg'
...
@borg.on(lightning_cmd(pattern="download(?: |$)(.*)", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(lightning_cmd(pattern="upload (.*)", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.reply("Processing ...")
...
# @borg.on(lightning_cmd(pattern="uploadas(stream|vn|all) (.*)", allow_sudo=True))
